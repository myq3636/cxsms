...
public class MKMessageHttpHandler extends HttpHandler {
	private static SystemLogger log = SystemLogger
			.getSystemLogger(MKMessageHttpHandler.class);

	public MKMessageHttpHandler(HttpInterface hie) {
		super(hie);
	}
	...
	public String makeRequest(GmmsMessage message, String urlEncoding,
		...
		for(HttpParam param:parameters){
			...
			if("recipientAddress".equalsIgnoreCase(pval)){
				postData.append("&").append(param.getOppsiteParam()).append("=").append(address[1]);//recipient
				...
				if(GmmsMessage.AIC_CS_UCS2.equalsIgnoreCase(message.getContentType())) {
					...
					for (int i = 0; i < content.length(); i++) {
						...
						if (c > 255) {
					          unicode.append(Integer.toHexString((int) c));
					    } else {
					          unicode.append("00" + Integer.toHexString((int) c));
					    }						
					}		
					postData.append("&").append(param.getOppsiteParam()).append("=").append(unicode.toString());//message
				}else {
				...
				postData.append("&").append(param.getOppsiteParam()).append("=").append(address[0]);//sender
			}else{
		...
		if(GmmsMessage.AIC_CS_UCS2.equalsIgnoreCase(message.getContentType())) {
			postData.append("&").append("type=5");
		}else {
			postData.append("&").append("type=0");
		}
		postData.append("&").append("detail=1");
		postData.append("&").append("servid=").append(cst.getSMSOptionHttpCustomParameter());
	...
	public void parseResponse(GmmsMessage msg, String resp) {
		log.info("receive Macrokiosk response is {}, ", resp);
		if (StringUtility.stringIsNotEmpty(resp)) {
			String response[] = resp.split(",");
			if(response!=null && response.length>=3) {
				String outmsgid = response[1];
				String twstatus = response[2];
