...
public class CommonMessageHttpHandler extends HttpHandler {
	private static SystemLogger log = SystemLogger
			.getSystemLogger(CommonMessageHttpHandler.class);

	public CommonMessageHttpHandler(HttpInterface hie) {
		super(hie);
	}
	...
	public String makeRequest(GmmsMessage message, String urlEncoding,
		...
		String postData =createXml(cst.getChlPasswordr(), message);
		//String postData = 
		return postData;
	}
	...
	public void parseResponse(GmmsMessage msg, String resp) {
		...
		if (!StringUtility.stringIsNotEmpty(resp)) {
			...
			HttpStatus status = new HttpStatus("0", resp.trim());
	...
	private static String createXml(String productToken, GmmsMessage message) {
        try {
        	StringWriter sw = new StringWriter();
        	 JxMessage jxMessage = new JxMessage();
        	 JxSubMessage jxSubMessage = new JxSubMessage();
        	 JxToken token = new JxToken();
        	 token.setPRODUCTTOKEN(productToken);
        	 jxSubMessage.setDCS(GmmsMessage.AIC_CS_UCS2.equals(message.getContentType())?"8":"0");
        	 jxSubMessage.setFROM(message.getSenderAddress());
        	 jxSubMessage.setBODY(message.getTextContent());
        	 jxSubMessage.setTO(message.getRecipientAddress());
        	 //jxSubMessage.setRSSID("16");
        	 if (message.getOutMsgID() == null) {
        		 String commonMsgID = MessageIdGenerator.generateCommonStringID();
				message.setOutMsgID(commonMsgID);
			  }
        	 jxSubMessage.setREFERENCE(message.getOutMsgID());
        	 jxMessage.setAUTHENTICATION(token);
	...
	public static void main(String[] args) {
		try {
			...
			//String URL = "http://39.106.43.83:7031/Gmms/YunXun/DRReply";
			//String URL = "http://39.104.174.46/Gmms/equii/DRReply";
            ...
            threads[i]=new Thread(){
            	...
            	public void run() {
            		for (int j = 0; j < 1; j++) {
                        ...
                        String token = "402_201712131308_test";
                    
                        //String URL = "http://39.104.174.46/Gmms/equii/DRReply";
                       //String URL = "http://39.104.174.46/Gmms/Common/MTRequest";
                        //String URL = "http://39.104.174.46/Gmms/cainiao/MTRequest";
                        //String URL = "http://8.210.140.175:8085/api/v1/message/sendtosim";
                        //String URL = "http://gwhk.jxintelink.com:11630/Gmms/CommonJson/MTRequest";
                        //String URL = "https://sgsms.jxintelink.com:16283/Gmms/Common/MTRequest";
                        if (j%2==0) {
                        	//URL = "http://39.104.174.46/Gmms/CommonJson/MTRequest";
                        	//URL = "http://39.104.174.46/Gmms/Common/MTRequest";
						}        
                        String URL = "https://msggw.jxintelink.com:1885/Gmms/cainiao/MTRequest";
                        //String URL = "http://39.104.174.46/Gmms/CommonJson/MTRequest";
                		GmmsMessage message = new GmmsMessage();
                    	message.setTextContent("现在预订房间有优惠吗？");
                        message.setSenderAddress("all");
                        message.setRecipientAddress("811142500200");
                        message.setOutMsgID("1000000023");
                        message.setContentType("UnicodeBigUnmarked");
                        String xml = createXml(token, message);                
                        //System.out.println(xml);
						/*
						 * String json = "{" +
						 * "\"content\":\"[11.11 SPECIAL INVITE: ONE-CLICK FAST ENROLLMENT]Hi seller, good news! We've made registration for 11.11 very FAST & EASY for you! We have sent a special invitation with inputted prices (already set at friendliest discount possible) already for you. All you need to do is review the invite & click \\\"Accept\\\" and you'll be good to go for 11.11. Follow simple steps below:1. Access Special Invite here: https://sellercenter.lazada.com.ph/apps/campaign/scene/detail?masterId=60&sceneId=302505&from=detail2. Review prices.3. Once good to go, click \\\"Accept\\\" in the invite.Please accept this special invite ASAP. Let me know when you are done. Once you're done with these steps, you'll officially be ready for 11.11.Thank you!\","
						 * +
						 * "\"extend\":\"&;9853762294;;2218497664110;0132;30;72330971322495195^322;2024-10-28\","
						 * + "\"method\":\"MD5\"," + "\"phoneNumber\":\"639853762294\"," +
						 * "\"requestId\":\"72330971324363500^398\"," + "\"senderId\":\"Lazada\"," +
						 * "\"signature\":\"pMhCmHQt3nqHkk20uBQLVQ==\"," + "\"supplier\":\"caLaMkt\"," +
						 * "\"timestamp\":\"1730107805469\"" + "}";
						 */
                        		
						/*
						 * String json = "{\"Sender\": \"8613523456789\", " +
						 * "\"Recipient\": \"91223485856\", " + "\"Charset\": 8, " +
						 * "\"Token\": \"402_201712131308_test\", " +
						 * "\"Content\": \"HTTPAPI测试请查收您的验证码123您的验证码第二条\"}";
						 */
                        String json = "{"
                        		+ " \"AUTHENTICATION\": { "
                        		+ " \"PRODUCTTOKEN\": \"83_201712131308_test\" "
                        		+ " }, "
                        		+ " \"MSG\":["
                        		+ " { "
                        		+ " \"FROM\": \"xjtls\", "
                        		+ " \"TO\": \"6281292237323\", "
                        		+ " \"DCS\": 0, "
                        		+ " \"REFERENCE\": \"234342343252\", "
                        		+ " \"BODY\": \"Want to save time and grow your business? Charistel smart software makes it simple! Get started today!\" "
                        		+ " }"
                        		+ " ]"
                        		+ "}";
                        /*String xml = "<xmlRequest>"
                        		+ "<header>"
                        		+ "<syscode>1234ddf</syscode>"
                        		+ "<depcode>000000</depcode>"
                        		+ "<depname>121212</depname>"
                        		+ "<password>123</password>"
                        		+ "</header>"
                        		+ "<body>"
                        		+ "<sms>"
                        		+ "<mobile>861850333</mobile>"
                        		+ "<content>短信内容</content>"
                        		+ "</sms></body>"
                        		+ "</xmlRequest>";
                        String json="{\"ToNumber\":\"17600019\","
                        		+ "\"Status\":\"Delivered\","
                        		+ "\"Message\":\"\",\"FromNumber\":\"99\","
                        		+ "\"Recipient_Type\":\"Phone\","
                        		+ "\"MessageID\":\"2345666\",\"Result\":\"Delivered Successfully\"}";*/
            ...
            /*for (int i = 0; i < 1; i++) {            	
               GmmsMessage message = new GmmsMessage();
                ...
                String json = "{\"AUTHENTICATION\": "
                		...
                		+ "\"TO\": \"8618500375454\", "
                		+ "\"DCS\": 8, "
                		+ "\"BODY\": \"【极讯科技】您的验证码123456， 十分钟有效，勿转发\"}]}";
	...
	/*public static void main(String[] args) {
		String outMsgID = "245567abc";
		System.out.println(Long.toString(Long.valueOf(outMsgID, 16)));;
	}*/				
}
