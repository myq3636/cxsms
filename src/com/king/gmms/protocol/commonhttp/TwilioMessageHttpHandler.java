...
public class TwilioMessageHttpHandler extends HttpHandler {
	private static SystemLogger log = SystemLogger
			.getSystemLogger(TwilioMessageHttpHandler.class);

	public TwilioMessageHttpHandler(HttpInterface hie) {
		super(hie);
	}
	...
	public String makeRequest(GmmsMessage message, String urlEncoding,
		...
		for(HttpParam param:parameters){
			...
			if("recipientAddress".equalsIgnoreCase(pval)){
				...
				postData.append("&").append(param.getOppsiteParam()).append("=").append(urlEncode(address[0], urlEncoding));//sender
			}else{
	...
	public void parseResponse(GmmsMessage msg, String resp) {
		log.info("receive Twilio response is {}, ", resp);
