...
public class TwilioTemplateMessageHttpHandler extends HttpHandler {
	private static SystemLogger log = SystemLogger
			.getSystemLogger(TwilioTemplateMessageHttpHandler.class);

	public TwilioTemplateMessageHttpHandler(HttpInterface hie) {
		super(hie);
	}
	...
	public String makeRequest(GmmsMessage message, String urlEncoding,
		...
		String friendName = null;
		 if(m.find()){
			 code = m.group();
			 friendName = textContent.substring(code.length());
		 }
		for(HttpParam param:parameters){
			...
			if("recipientAddress".equalsIgnoreCase(pval)){
				...
				postData.append("&").append(param.getOppsiteParam()).append("=").append(code);//message
			}else if("senderAddress".equalsIgnoreCase(pval)){			
				postData.append("&").append(param.getOppsiteParam()).append("=").append(urlEncode(friendName,urlEncoding));//sender
			}else{
	...
	public void parseResponse(GmmsMessage msg, String resp) {
		log.info("receive Twilio template response is {}, ", resp);
		if (StringUtility.stringIsNotEmpty(resp)) {
			JSONObject obj = JSONObject.parseObject(resp);
			String twstatus = obj.getString("status");
			String outmsgid = obj.getString("sid");
