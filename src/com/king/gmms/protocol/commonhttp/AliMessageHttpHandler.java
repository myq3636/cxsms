...
public class AliMessageHttpHandler extends HttpHandler {
	private static SystemLogger log = SystemLogger
			.getSystemLogger(AliMessageHttpHandler.class);

	public AliMessageHttpHandler(HttpInterface hie) {
		super(hie);
	}
		...
		*/
		...
		for(HttpParam param:parameters){
			...
			if("recipientAddress".equalsIgnoreCase(pval)){
				postData.append("&").append(param.getOppsiteParam()).append("=").append(SMSSignatureUtil.percentEncode(address[1]));//recipient
			}else if("textContent".equalsIgnoreCase(pval)){
				postData.append("&").append(param.getOppsiteParam()).append("=").append(SMSSignatureUtil.percentEncode(message.getTextContent()));//message
			}else if("senderAddress".equalsIgnoreCase(pval)){			
				postData.append("&").append(param.getOppsiteParam()).append("=").append(SMSSignatureUtil.percentEncode(address[0]));//sender
			}else{
				String mval = HttpUtils.getParameter(param, message, cst);
				customValue = SMSSignatureUtil.percentEncode(mval);
				postData.append("&").append(param.getOppsiteParam()).append("=").append(SMSSignatureUtil.percentEncode(mval));
			}
		}
		String accessKey = cst.getChlAcctNamer();
		String accessSec = cst.getChlPasswordr();
		String timestamp = SMSSignatureUtil.formatIso8601Date(new Date());
		String signatureNonce = UUID.randomUUID().toString();
		String signature = SMSSignatureUtil.getSignature(accessKey, sender, content, recipient, accessSec, signatureNonce, timestamp, customValue);
		postData.append("&").append("Version=2018-05-01");
		postData.append("&").append("SignatureMethod=HMAC-SHA1");
		postData.append("&").append("SignatureVersion=1.0");
		postData.append("&").append("Action=SendMessageToGlobe");
		postData.append("&").append("Format=JSON");
		postData.append("&").append("Timestamp=").append(SMSSignatureUtil.percentEncode(timestamp));
		postData.append("&").append("AccessKeyId=").append(accessKey);
		postData.append("&").append("SignatureNonce=").append(SMSSignatureUtil.percentEncode(signatureNonce));
		postData.append("&").append("Signature=").append(SMSSignatureUtil.percentEncode(signature));
	...
	public void parseResponse(GmmsMessage msg, String resp) {
		log.info("receive Ali response is {}, ", resp);
		if (StringUtility.stringIsNotEmpty(resp)) {
			JSONObject obj = JSONObject.parseObject(resp);
			String twstatus = obj.getString("ResponseCode");
			String outmsgid = obj.getString("MessageId");
			HttpStatus status = new HttpStatus(twstatus, twstatus);
